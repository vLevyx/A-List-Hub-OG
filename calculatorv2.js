...
function addToKit() {
  ...
  // 🔁 Check for duplicates and increment quantity if found
...
function renderKit() {
  ...
  kit.forEach((entry, index) => {
    const li = document.createElement("li");
    li.innerHTML = `<strong>${entry.item}</strong> (x${entry.quantity}) 
      <button class='remove-btn' onclick='removeFromKit(${index})'>Remove</button>`;
    ul.appendChild(li);
  });
}
